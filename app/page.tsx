...
function LoadingOverlay({ onDone }: { onDone: () => void }) {
  ...
  const full = "INITIALIZING KUBEVIEW...";
  useEffect(() => {
    let i = 0;
    const t = setInterval(() => {
      i++;
...
function HeroCanvas({ ready }: { ready: boolean }) {
  ...
  useEffect(() => {
    ...
    ro.observe(canvas);
    // Also set immediately in case already laid out
    if (canvas.offsetWidth > 0) { canvas.width = canvas.offsetWidth; canvas.height = canvas.offsetHeight; }

    // Node positions as fractions (0-1) — computed into pixels in draw()
    ...
    let pathT = 0;
    let pulseNode = -1; let pulseTime = 0;
    let dashOffset = 0; let t = 0;
    // Per-frame computed node positions
    ...
    const draw = () => {
      ...
      // Compute pixel positions per frame (relative → absolute)
      ...
      nodeDefs.forEach((d, i) => {
        ...
        } else if (d.type === "secret") { ctx.fillRect(-7,-7,14,14);
        } else if (d.type === "db") {
          ctx.beginPath();
  ...
  return <canvas ref={canvasRef} id="hero-canvas" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} />;
}
  ...
  return (
    <section style={{ position: "relative", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "radial-gradient(ellipse at 50% 50%, rgba(249,115,22,0.03) 0%, transparent 70%)" }}>
      ...
      <div style={{ position: "relative", zIndex: 10, textAlign: "center", maxWidth: 800, padding: "0 20px" }}>
        ...
        <div style={{ display: "flex", gap: 24, justifyContent: "center", marginTop: 24, opacity: wordsIn ? 1 : 0, transition: "opacity 600ms 700ms" }}>
          {["✓ No data leaves your cluster", "✓ Works with any K8s provider", "✓ Free to start"].map(t => (
            <span key={t} style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--muted)" }}><span style={{ color: "var(--green)" }}>✓</span> {t.slice(2)}</span>
          ))}
        </div>
      </div>
    </section>
...
/* ─── SECTION 2: ATTACK GRAPH BUILD (sticky 400vh) ─── */
function AttackGraphSection() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const prog = Math.max(0, Math.min(1, (-rect.top) / (sH - window.innerHeight)));
      ...
      if (ph !== termPhaseRef.current) {
        ...
        let i = 0; setTermText("");
        const q = queries[ph];
        const t = setInterval(() => { i++; setTermText(q.slice(0, i)); if (i >= q.length) clearInterval(t); }, 40);
      }
    };
  ...
  useEffect(() => {
    ...
    if (canvas.offsetWidth > 0) { canvas.width = canvas.offsetWidth; canvas.height = canvas.offsetHeight; }
    const gNodes = [
      {x:0.15,y:0.3,label:"nginx-pod",color:"#00f5ff"},{x:0.35,y:0.2,label:"api-pod",color:"#00f5ff"},
      {x:0.55,y:0.3,label:"sa-admin",color:"#fff"},{x:0.75,y:0.2,label:"role-read",color:"#39ff14"},
      {x:0.2,y:0.5,label:"sa-default",color:"#fff"},{x:0.45,y:0.55,label:"secret-db",color:"#ff3131"},
      {x:0.65,y:0.5,label:"config-env",color:"#9b5de5"},{x:0.3,y:0.75,label:"db-prod",color:"#ffd700"},
      {x:0.6,y:0.75,label:"db-staging",color:"#ffd700"},{x:0.85,y:0.4,label:"sa-Mon",color:"#fff"},
      {x:0.1,y:0.7,label:"role-admin",color:"#39ff14"},{x:0.5,y:0.85,label:"sec-cred",color:"#ff3131"},
    ];
    const gEdges = [[0,2],[1,2],[2,3],[4,5],[5,7],[3,6],[6,8],[0,10],[10,7],[9,6],[1,4],[4,11],[11,8]];
    const attackPath = [0,2,3,6,8];
    let traceT = 0; let af: number;
    const draw = () => {
      const W = canvas.width, H = canvas.height;
      if (!W || !H) { af = requestAnimationFrame(draw); return; }
      const prog = progressRef.current;
      ctx.clearRect(0,0,W,H);
      const nodeCount = Math.floor(Math.min(prog / 0.33, 1) * gNodes.length);
      const nx = (n: typeof gNodes[0]) => n.x * W;
      const ny = (n: typeof gNodes[0]) => n.y * H;
      gNodes.slice(0,nodeCount).forEach((n,i) => {
        const alpha = Math.min(1,(prog/0.33*gNodes.length-i));
        ctx.save(); ctx.globalAlpha = Math.max(0,alpha);
        ctx.fillStyle = n.color; ctx.shadowColor = n.color; ctx.shadowBlur = 10;
        ctx.beginPath(); ctx.arc(nx(n),ny(n),7,0,Math.PI*2); ctx.fill();
        ctx.fillStyle="rgba(240,240,240,.7)"; ctx.font="10px monospace"; ctx.shadowBlur=0; ctx.textAlign="left";
        ctx.fillText(n.label, nx(n)+10, ny(n)+4); ctx.restore();
      });
      if (prog > 0.33) {
        const ep = (prog-0.33)/0.33;
        gEdges.forEach(([a,b]) => {
          if (a>=nodeCount || b>=nodeCount) return;
          const na=gNodes[a], nb=gNodes[b];
          const len=Math.hypot(nx(nb)-nx(na),ny(nb)-ny(na));
          const drawn=Math.min(ep*len,len);
          const ang=Math.atan2(ny(nb)-ny(na),nx(nb)-nx(na));
          ctx.save(); ctx.strokeStyle="rgba(0,245,255,.2)"; ctx.lineWidth=1; ctx.setLineDash([4,4]);
      ...
      if (prog > 0.66) {
        traceT = Math.min(traceT+0.008,1);
        ...
        if(seg<attackPath.length-1){
          ...
          const tx=nx(na)+(nx(nb)-nx(na))*frac, ty=ny(na)+(ny(nb)-ny(na))*frac;
          ctx.save(); ctx.fillStyle="#00f5ff"; ctx.shadowColor="#00f5ff"; ctx.shadowBlur=12;
          ctx.beginPath(); ctx.arc(tx,ty,5,0,Math.PI*2); ctx.fill(); ctx.restore();
        }
        attackPath.slice(0,Math.floor(traceT*attackPath.length)+1).forEach(idx=>{
          const n=gNodes[idx];
          ctx.save(); ctx.fillStyle="#ff3131"; ctx.shadowColor="#ff3131"; ctx.shadowBlur=10;
          ctx.beginPath(); ctx.arc(nx(n),ny(n),9,0,Math.PI*2); ctx.fill(); ctx.restore();
        });
      } else { traceT=0; }
      af=requestAnimationFrame(draw);
    };
  ...
  const texts=[
    {title:"MAPPING YOUR CLUSTER",body:"Kubeview ingests your kubeconfig and indexes every resource in real time."},
    {title:"DISCOVERING RELATIONSHIPS",body:"Every permission, binding, and mount rendered as a navigable edge in the attack graph."},
    {title:"TRACING ATTACK PATHS",body:"Graph traversal reveals how an attacker chains permissions from entry point to crown jewel."},
  ];
  return (
    <div ref={outerRef} style={{height:"400vh",position:"relative"}}>
      <div style={{position:"sticky",top:0,height:"100vh",display:"flex",overflow:"hidden",background:"var(--bg)"}}>
        <div style={{width:"40%",display:"flex",alignItems:"center",justifyContent:"center",padding:"0 48px",position:"relative"}}>
          {texts.map((t,i)=>(
            <div key={i} style={{position:"absolute",width:"80%",transition:"all .3s",opacity:phase===i?1:0,transform:phase===i?"none":phase<i?"translateX(40px)":"translateX(-40px)"}}>
              <div style={{fontFamily:"var(--mono)",fontSize:11,color:"var(--cyan)",letterSpacing:3,marginBottom:16}}>0{i+1}</div>
              <h2 style={{fontSize:"clamp(20px,2.5vw,28px)",fontWeight:700,marginBottom:16}}>{t.title}</h2>
              <p style={{fontSize:16,color:"var(--muted)",lineHeight:1.7}}>{t.body}</p>
            </div>
          ))}
        </div>
        <div style={{width:"60%",height:"100%",position:"relative",display:"flex",flexDirection:"column"}}>
          <canvas ref={canvasRef} style={{flex:1,display:"block",width:"100%"}}/>

          <div style={{position:"absolute",bottom:32,right:32,background:"#0a0a0a",border:"1px solid var(--border)",borderRadius:6,padding:"12px 16px",fontFamily:"var(--mono)",fontSize:11,color:"var(--cyan)",maxWidth:360}}>
            <span style={{color:"var(--muted)"}}>{">"} </span>{termText}<span style={{animation:"blink .8s infinite"}}>_</span>
          </div>
          {phase===2&&(
            <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",background:"rgba(255,49,49,.08)",border:"2px solid var(--red)",borderRadius:8,padding:"16px 24px",fontFamily:"var(--mono)",fontSize:13,color:"var(--red)",animation:"blink 2s infinite",textAlign:"center",boxShadow:"0 0 30px rgba(255,49,49,.3)",whiteSpace:"nowrap"}}>
...
/* ─── SECTION 3: CAPABILITIES  (horizontal sticky 500vh) ─── */
function CapabilitiesSection() {
  ...
  useEffect(()=>{
    const onScroll=()=>{
      ...
      const rect=outerRef.current.getBoundingClientRect();
      const sH=outerRef.current.offsetHeight;
      setProgress(Math.max(0,Math.min(1,(-rect.top)/(sH-window.innerHeight))));
    };
    window.addEventListener("scroll",onScroll,{passive:true});
    return ()=>window.removeEventListener("scroll",onScroll);
  },[]);
  ...
  useEffect(()=>{
    ...
    let af:number; let frame=0;
    const nodes=[{x:.5,y:.08,l:"ENTRY",c:"#ff3131"},{x:.3,y:.25,l:"api",c:"#00f5ff"},{x:.7,y:.25,l:"nginx",c:"#00f5ff"},{x:.15,y:.44,l:"sa-adm",c:"#fff"},{x:.5,y:.44,l:"role",c:"#39ff14"},{x:.75,y:.44,l:"secret",c:"#ff3131"},{x:.5,y:.7,l:"DB-PROD",c:"#ffd700"}];
    const edges=[[0,1],[0,2],[1,3],[1,4],[2,5],[4,6],[5,6]];
    const ap=new Set([0,1,4,6]);
    const draw=()=>{
      const W=cv.width,H=cv.height;
      if(!W||!H){af=requestAnimationFrame(draw);return;}
      ctx.clearRect(0,0,W,H); frame=(frame+1)%180;
      const show=Math.min(Math.floor(frame/180*nodes.length)+1,nodes.length);
      edges.forEach(([a,b])=>{
        if(a>=show||b>=show)return;
        const on=ap.has(a)&&ap.has(b);
        ctx.save();ctx.strokeStyle=on?"#00f5ff":"rgba(255,255,255,.12)";ctx.lineWidth=on?2:1;
      ...
      nodes.slice(0,show).forEach(n=>{
        ctx.save();ctx.fillStyle=n.c;ctx.shadowColor=n.c;ctx.shadowBlur=8;
        ctx.beginPath();ctx.arc(n.x*W,n.y*H,14,0,Math.PI*2);ctx.fill();
  ...
  const panels=[
    {tag:"CAPABILITY 01",title:"Attack Path Discovery",body:"BFS graph traversal from every entry point to every crown jewel — ranked by exploitability.",stat:"23 paths found in 240ms",ref:c1},
    {tag:"CAPABILITY 02",title:"RBAC Relationship Mapping",body:"Every ServiceAccount, Role, and binding as navigable edges. Find wildcard permissions instantly.",stat:"100% of bindings mapped",ref:c2},
    {tag:"CAPABILITY 03",title:"CVE Enrichment",body:"Running image SHAs matched against NVD and OSV databases. Vulnerabilities anchored to nodes.",stat:"99.7% NVD match rate",ref:c3},
    {tag:"CAPABILITY 04",title:"Crown Jewel Detection",body:"Automatically identify Secrets, ConfigMaps, and database pods representing the highest-value targets.",stat:"3 crown jewels auto-detected",ref:c4},
  ];
  return (
    <div ref={outerRef} style={{height:"500vh",position:"relative"}}>
      <div style={{position:"sticky",top:0,height:"100vh",overflow:"hidden",background:"var(--bg)"}}>
        <div style={{display:"flex",width:"400vw",height:"100%",transform:`translateX(-${progress*300}vw)`,willChange:"transform"}}>
          {panels.map((p,pi)=>(
            <div key={pi} style={{width:"100vw",height:"100%",flexShrink:0,display:"flex",alignItems:"center",padding:"0 clamp(30px,5vw,60px)",gap:"clamp(30px,4vw,60px)",position:"relative"}}>
              <div style={{width:"45%",height:"60%"}}>
                <canvas ref={p.ref} style={{width:"100%",height:"100%",border:"1px solid var(--border)",borderRadius:8}}/>
              </div>
              <div style={{width:"45%"}}>
                <div style={{fontFamily:"var(--mono)",fontSize:11,color:"var(--cyan)",letterSpacing:3,marginBottom:16}}>{p.tag}</div>
                <h2 style={{fontSize:"clamp(28px,3vw,40px)",fontWeight:800,letterSpacing:-1,marginBottom:20,lineHeight:1.1}}>{p.title}</h2>
                <p style={{fontSize:17,color:"var(--muted)",lineHeight:1.7,marginBottom:24}}>{p.body}</p>
                <div style={{fontFamily:"var(--mono)",fontSize:22,color:"var(--cyan)",fontWeight:700}}>{p.stat}</div>
              </div>
              {pi<panels.length-1&&<div style={{position:"absolute",right:0,top:0,width:40,height:"100%",background:"repeating-linear-gradient(-45deg,rgba(0,245,255,.05),rgba(0,245,255,.05) 1px,transparent 1px,transparent 8px)"}}/>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{position:"relative",width:"min(1100px,90vw)"}}>
        ...
        <div style={{background:"#0d0d0d",borderRadius:12,overflow:"hidden",boxShadow:"0 0 80px rgba(0,245,255,.12),0 60px 120px rgba(0,0,0,.9)",animation:"floatY 4s ease-in-out infinite",border:"1px solid rgba(255,255,255,.08)"}}>
          <div style={{height:44,background:"#1a1a1a",display:"flex",alignItems:"center",padding:"0 16px",gap:8}}>
            ...
            <div style={{flex:1,textAlign:"center",background:"#2a2a2a",borderRadius:4,padding:"4px 16px",fontFamily:"var(--mono)",fontSize:11,color:"var(--muted)",margin:"0 60px"}}>kubeview.app/dashboard</div>
          </div>
          <div style={{height:480,background:"#050508",display:"flex",alignItems:"center",justifyContent:"center",backgroundImage:"linear-gradient(rgba(0,245,255,.03) 1px,transparent 1px),linear-gradient(90deg,rgba(0,245,255,.03) 1px,transparent 1px)",backgroundSize:"40px 40px"}}>
            <div style={{textAlign:"center"}}>
              <div style={{fontFamily:"var(--mono)",fontSize:"clamp(20px,3vw,32px)",fontWeight:800,color:"var(--cyan)",letterSpacing:4,marginBottom:16,opacity:.3}}>KUBEVIEW DASHBOARD</div>
...
const TERM_LINES=[
  {t:"$ kubeview scan --cluster production --output graph",c:"#f0f0f0",b:false},
  {t:"",c:"",b:false},
  ...
  {t:"→ kubeview.app/report/production-2025",c:"rgba(240,240,240,.5)",b:false},
  {t:"",c:"",b:false},
...
function TerminalSection(){
  ...
  const run=useCallback(()=>{
    ...
    const next=()=>{
      ...
      let ci=0;
      const type=()=>{ci++;setShown(p=>{const c=[...p];if(c.length)c[c.length-1]={...c[c.length-1],t:line.t.slice(0,ci)};return c;});
    ...
    <section ref={inViewRef} className="kvs" style={{minHeight:"100vh",display:"grid",gridTemplateColumns:"1fr 1fr",gap:60,alignItems:"center",padding:"80px 60px",maxWidth:1280,margin:"0 auto"}}>
      ...
      <div style={{background:"#0a0a0a",borderRadius:10,border:"1px solid rgba(255,255,255,.08)",overflow:"hidden"}}>
        <div style={{background:"#141414",padding:"10px 16px",display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{flex:1,textAlign:"center",fontFamily:"var(--mono)",fontSize:11,color:"var(--muted)"}}>kubeview — zsh</span>
        </div>
  ...
  return(
    <section className="kvs" style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 60px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:24,maxWidth:1100,width:"100%",position:"relative"}}>
        ...
        <HowCard num="01" icon="⬡" title="Connect" body="Point Kubeview at your kubeconfig. Read-only. No data leaves your cluster." tag="< 30 seconds" delay="0ms" spin={false} pulse={false}/>
  ...
  return(
    <footer style={{borderTop:"1px solid var(--border)",position:"relative"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"60px 40px 40px",display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:40}}>
        <div>
          <div style={{fontFamily:"var(--mono)",fontSize:20,fontWeight:700,color:"var(--cyan)",letterSpacing:2,marginBottom:8}}>KUBEVIEW</div>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
          ...
          <div style={{fontFamily:"var(--mono)",fontSize:11,color:"var(--muted)"}}>© 2025 Kubeview</div>
